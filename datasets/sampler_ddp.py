...
class DistributedRandomIdnumSampler(Sampler):
    """
    DDP compatible sampler: Randomly sample N identities,
    and K instances per identity. Total batch size = N * K.
    """
    ...
    def __init__(self, data_source, batch_size, num_instances, num_replicas=None, rank=None):
        ...
        # 最终样本数应能被 batch_size 整除
        samples_per_batch = self.batch_size // self.num_replicas
        self.num_samples = self.num_samples - (self.num_samples % samples_per_batch)
    ...
    def __iter__(self):
        # Ensure each epoch gets different shuffle
        random.seed(self.epoch)

        batch_idxs_dict = defaultdict(list)
        for pid in self.pids:
            idxs = copy.deepcopy(self.index_dic[pid])
            num_instances = random.randint(1, self.num_instances)
        ...
        # Ensure all ranks get equal length
        final_idxs = final_idxs[:self.num_samples * self.num_replicas]
        final_idxs = final_idxs[self.rank::self.num_replicas]
        return iter(final_idxs)
