...
def extract_features(model, data_loader):
    ...
    with torch.no_grad():
        for i, batch in enumerate(data_loader):
            image_feats, i_feats, t_feats = model(batch)
            for i_feat, t_feat, pid in zip(i_feats, t_feats, batch['pids']):
                features_i.append(i_feat)
                features_t.append(t_feat)
                labels_all.append(int(pid))
    model.train()
    return features_i, features_t, labels_all


def initial_classifier(model, data_loader):
    pid2features = collections.defaultdict(list)
    features_i, features_t, labels_all = extract_features(model, data_loader)
    for feature, pid in zip(features_i, labels_all):
