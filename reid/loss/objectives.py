...
class TripletLoss(nn.Module):
    ...
    def forward(self, anchor, positive, negative):
        distance_positive = 1 - cosine_similarity(anchor, positive)
        distance_negative = 1 - cosine_similarity(anchor, negative)
...
def get_similarity_matrix(image_norm, text_norm, logit_scale):
    ...
    # 释放中间张量
    del image_norm, text_norm, t2i_cosine_theta, i2t_cosine_theta, text_proj_image, image_proj_text
    torch.cuda.empty_cache()
    
    return i2t_pred, t2i_pred


def compute_tri(anchor, positive, negative, margin):
