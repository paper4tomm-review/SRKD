...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, need_mae=False, mask_ratio=0.6):
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, need_mae=False, mask_ratio=0.6):
        return self.visual(image.type(self.dtype), need_mae=need_mae, mask_ratio=mask_ratio)
    ...
    def forward(self, image, text, need_MAE=False, mask_ratio=0.6):
        if need_MAE:
